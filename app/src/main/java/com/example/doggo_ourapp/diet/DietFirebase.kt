...
import com.example.doggo_ourapp.diet.DietFirebase.loadMaxNutrientsForDiet
import com.example.doggo_ourapp.diet.DietFirebase.loadTotalNutrientsForDiet
    ...
    fun clearDietIfNewDay(onResult: (Boolean) -> Unit) {
        ...
        DogFirebase.getActualDog { dogId ->
            if (dogId == null) {
                ...
                checkIfDogHasDiet { hasDiet ->
                    if (!hasDiet) {
                        onResult(false)
                        return@checkIfDogHasDiet
                    }
